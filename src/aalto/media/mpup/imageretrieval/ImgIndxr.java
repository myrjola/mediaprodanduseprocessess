 ...
 * 
 * Lux Mathias, Savvas A. Chatzichristofis. 
...
public class ImgIndxr {
	...
	public static String createIndex(String imageFolder) throws IOException {    
	    ...
	    if (!IndexReader.indexExists(FSDirectory.open(lireIndexFolder))) {
		    ...
		    IndexWriter iw = new IndexWriter(FSDirectory.open(lireIndexFolder), conf);
		    ...
		    for (File imageFile : imageFiles) {
		        ...
		        System.out.print("Indexing image (" + i + "/" + imgs + "): " + imageFilePath);
				...
				if (xmpString != null) {
					try {
						...
						String dc_identifier = xmp.getPropertyString(XMPConst.NS_DC, "identifier");
		        ...
		        System.out.println("Done in " + elapsed + " ms. " + " (" +
		        		"Elapsed time " + ((float)timeElapsed / 60000) + " min. " +
		        				"Estimated time left " + 
		        				(((float)avgImageProcessingTime * (imgs - i)) / 60000) + " min.)");
		    }
		    iw.close();
	    } 
